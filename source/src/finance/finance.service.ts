...
export class FinanceService {
  ...
  createFinance({
    ...
    console.log(
      `A new finance record with ID#${unique_id} has been added successfully.`
    );
    return this.financeClient.emit(
      "finance_list_created",
      new CreatedFinanceRecord(
        autoGenerateId,
        unique_id,
        source_full_name,
        destination_full_name,
  ...
  createMobleService({
    ...
    payment_status,
    date,
  }: addUserRequestInFinance) {
    return this.financeClient.emit(
      "finance_user_created",
      new CreatedUserInFinance(id, name, phone, payment_status, date)
    );
  }
}
