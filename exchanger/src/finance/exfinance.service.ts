...
export class exchangeFinanceService{
    ...
    handleRecordCreated({createdId, unique_id, source_full_name, destination_full_name, amount, currency}: CreatedFinanceRecord){
        // console.log("Exchange Finance Record has been created successfully", currency+" "+ amount)
        const recievedDate = new Date(); 
        return this.authClient.send('finance_list', new CreatedFinanceRequest(createdId, unique_id, source_full_name, destination_full_name, amount, currency, `${recievedDate}`))
        .subscribe((user) => {
            console.log(
    ...
    userCreatedinFinanceService({id, name}: CreatedUserInFinance){
        ...
        return this.authClient.send('finance_user_list', new AddedUserInFinance(id, name, `${getDate}`, ))
    }
}